...
class VisionLSTM2(nn.Module):
    ...
    def forward(self, x):
        x = self.patch_embed(x) #[b, 14, 14, 192]
        x = self.pos_embed(x)  #[b, 14, 14, 192]
        x = einops.rearrange(x, "b ... d -> b (...) d") #[b, 196, 192]

        for block in self.blocks:
            x = block(x) #[b, 196, 192]
        
        x = self.norm(x) #[b, 196, 192]
...
if __name__ == "__main__":
    model2 = torch.hub.load("nx-ai/vision-lstm", "vil2-base")
    model2.patch_embed = torch.nn.Identity()
    model2.pos_embed = torch.nn.Identity()
    model2.head = torch.nn.Linear(1536, 768)
